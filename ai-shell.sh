...
# ai-shell is read-only. One-shots get a Bash tool so they can *inspect*
# (list files, read state), but it runs under claude's plan permission mode,
# which mechanically denies any mutating command — nothing a one-shot does
# can change the filesystem. The prompt rules keep the model from trying,
# so the mechanical denial is a backstop, not the normal path.
#
...
_ai_flags() {
  _ai_flags_out=(-p --safe-mode --model "$ASK_MODEL" --tools "Bash" --permission-mode plan)
